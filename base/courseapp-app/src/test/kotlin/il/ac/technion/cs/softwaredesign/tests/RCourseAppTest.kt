...
class RCourseAppTest {
    ...
    fun `adding listener and removes it with bad callback lambda throws NoSuchEntityException`() {
        ...
        assertThrows<NoSuchEntityException> { courseApp.removeListener(adminToken, otherCallBack).joinException() }
    }
    ...
    fun `adding listener and broadcast successfully with the right broadcast source string`() {
        ...
        courseApp.addListener(adminToken, callBack).join()
        courseApp.broadcast(adminToken, messageFactory.create(MediaType.LOCATION, "Israel".toByteArray()).get()).join()
    ...
    fun `unable to fetch messages that was after channel deleted and created again`() {
        ...
        assertDoesNotThrow { courseApp.fetchMessage(adminToken, message1.id).join() }
    }
